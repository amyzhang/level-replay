...
if __name__ == "__main__":
    ...
    if args.train_full_distribution:
        train_seeds = None
    else:
        if args.seed_path:
            train_seeds = load_seeds(args.seed_path)
        elif args.use_env_name_seed_file:
            seed_files = [f for f in os.listdir('seeds/') if f.startswith(args.env_name)]
            if len(seed_files) > 0:
                train_seeds = load_seeds(os.path.join('seeds', seed_files[0]))
            else:
                raise ValueError(f"Seed file starting with {args.env_name} not found in seeds/")
        else:
